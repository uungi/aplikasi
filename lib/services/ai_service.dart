...
- Nama: ${InputSanitizer.sanitizeName(input.name)}
- Posisi: ${InputSanitizer.sanitizeText(input.position)}
- Pengalaman: ${InputSanitizer.sanitizeText(input.experiences)}
- Pendidikan: ${InputSanitizer.sanitizeText(input.education)}
- Keahlian: ${InputSanitizer.sanitizeText(input.skills)}
- Kontak: ${InputSanitizer.sanitizeText(input.contact)}
${input.additional.isNotEmpty ? "- Lain-lain: ${InputSanitizer.sanitizeText(input.additional)}" : ""}
...
- Keahlian: ${InputSanitizer.sanitizeText(input.skills)}
- Kontak: ${InputSanitizer.sanitizeText(input.contact)}
${input.company.isNotEmpty ? "- Perusahaan tujuan: ${InputSanitizer.sanitizeText(input.company)}" : ""}
${input.additional.isNotEmpty ? "- Lain-lain: ${InputSanitizer.sanitizeText(input.additional)}" : ""}
...
''';
  ...
  void _validateAndSanitizeInput(UserInput input) {
    // Check for suspicious content
    final inputs = [input.name, input.position, input.experiences, input.education, input.skills, input.contact, input.additional, input.company];
    
    for (final inputText in inputs) {
      if (InputSanitizer.containsSuspiciousContent(inputText)) {
        throw SecurityException('Input contains potentially harmful content');
      }
    }

    // Validate required fields
    final nameValidation = Validators.validateName(input.name);
    if (nameValidation != null) {
      throw ValidationException(nameValidation);
    }

    final positionValidation = Validators.validatePosition(input.position);
    if (positionValidation != null) {
      throw ValidationException(positionValidation);
    }

    final contactValidation = Validators.validateContact(input.contact);
    if (contactValidation != null) {
      throw ValidationException(contactValidation);
    }
  }
  ...
  Future<String> _generateOfflineResume(UserInput input) async {
    ...
    try {
      ...
      content = content.replaceAll('[Full Name]', InputSanitizer.sanitizeName(input.name));
      content = content.replaceAll('[Your Name]', InputSanitizer.sanitizeName(input.name));
      content = content.replaceAll('[Position]', InputSanitizer.sanitizeText(input.position));
      content = content.replaceAll('[Your Position/Title]', InputSanitizer.sanitizeText(input.position));
      content = content.replaceAll('[Contact Information]', InputSanitizer.sanitizeText(input.contact));
  ...
  Future<String> _generateOfflineCoverLetter(UserInput input) async {
    ...
    try {
      ...
      if (input.company.isNotEmpty) {
        content = content.replaceAll('[Company Name]', InputSanitizer.sanitizeText(input.company));
      } else {
...
''';
  ...
  Future<String> _sendPrompt(String prompt, {int maxRetries = 3}) async {
    int retryCount = 0;
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          throw ApiKeyException('Invalid API key. Please check your OpenAI API key.');
        } else {
          debugPrint('API Error: ${response.statusCode} - ${response.body}');
          throw AIServiceException('Failed to generate content: ${response.statusCode}');
        }
      } catch (e) {
        if (e is ApiKeyException) {
          rethrow; // Don't retry for API key issues
        }
        
        if (retryCount == maxRetries - 1) {
          debugPrint('Error generating content after $maxRetries attempts: $e');
          throw AIServiceException('Network error after multiple attempts: $e');
        }
        retryCount++;
    ...
    throw AIServiceException('Failed to generate content after $maxRetries attempts');
  }
}
